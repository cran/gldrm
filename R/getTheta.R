## Log likelihood function included here because it uses output from getTheta
getLoglik <- function(ySptIndex, fTilt) sum(log(fTilt[cbind(ySptIndex, seq_along(ySptIndex))]))
...
#'
#' @return List containing theta, b (sum(exp(theta*spt))), bPrime (the fitted
#' means, which should match mu), bPrime2 (the fitted variances), and tiltFact
#' (sptN x n matrix).
#'
#' @keywords internal
getTheta <- function(spt, f0, mu, thetaStart=NULL, thetaControl=theta.control())
{
    ...
    list(theta=theta, bPrime=bPrime, bPrime2=bPrime2, fTilt=fTilt, conv=conv, iter=iter)
}
